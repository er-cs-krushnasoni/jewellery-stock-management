...
// const { decryptData } = require("../utils/encrypt"); // Add this line
...
//   if (!category || category.grossWeight === 0) return 0;
...
//     // Calculate averages for each metal with proper rounding
//     const goldAvgPurity = metadata.totalGoldGross > 0 
//       ? Math.floor((metadata.totalGoldPure / metadata.totalGoldGross) * 10000) / 100
//       : 0;
    
//     const silverAvgPurity = metadata.totalSilverGross > 0 
//       ? Math.floor((metadata.totalSilverPure / metadata.totalSilverGross) * 10000) / 100
//       : 0;

//     // Process categories by metal type
//     const goldCategories = {};
//     const silverCategories = {};

//     // Process categories from categoryTotals
//     if (metadata.categoryTotals) {
//       Object.entries(metadata.categoryTotals).forEach(([categoryName, categoryData]) => {
//         if (categoryData.metal === 'gold') {
//           goldCategories[categoryName] = {
//             grossWeight: categoryData.grossWeight || 0,
//             pureWeight: categoryData.pureWeight || 0,
//             totalItems: categoryData.totalItems || 0,
//             averagePurity: categoryData.grossWeight > 0 
//               ? Math.floor((categoryData.pureWeight / categoryData.grossWeight) * 10000) / 100
//               : 0
//           };
//         } else if (categoryData.metal === 'silver') {
//           silverCategories[categoryName] = {
//             grossWeight: categoryData.grossWeight || 0,
//             pureWeight: categoryData.pureWeight || 0,
//             totalItems: categoryData.totalItems || 0,
//             averagePurity: categoryData.grossWeight > 0 
...
//     const stockReport = {
//       gold: {
//         totalGross: metadata.totalGoldGross || 0,
//         totalPure: metadata.totalGoldPure || 0,
//         averagePurity: goldAvgPurity,
//         categories: goldCategories
//       },
//       silver: {
//         totalGross: metadata.totalSilverGross || 0,
//         totalPure: metadata.totalSilverPure || 0,
...
//         const decryptedSale = {
//           ...sale,
//           // Add decrypted fields to top level for processing
...
//     console.log("Successfully decrypted sales:", decryptedSales.length);

//     // Now perform aggregation on decrypted data using JavaScript
...
//       aggregation.totalItems += sale.isBulk ? (sale.itemCount || 1) : 1;
...
//       ...item,
//       averagePrice: item.salesCount > 0 ? item.totalSalesAmount / item.salesCount : 0,
//       totalPureWeight: item.totalGrossWeight * (item._id.purity / 100),
...
//       totalItems: decryptedSales.reduce((sum, sale) => sum + (sale.isBulk ? (sale.itemCount || 1) : 1), 0),
//       uniqueCustomers: new Set(decryptedSales.map(sale => sale.customerMobile).filter(mobile => mobile)).size,
//       averageTransactionValue: decryptedSales.length > 0 ? decryptedSales.reduce((sum, sale) => sum + sale.salesPrice, 0) / decryptedSales.length : 0,
//       allSalesPrices: decryptedSales.map(sale => sale.salesPrice),
//       minSalesPrice: Math.min(...decryptedSales.map(sale => sale.salesPrice)),
//       maxSalesPrice: Math.max(...decryptedSales.map(sale => sale.salesPrice))
//     };
...
//       customer.totalPurchases += sale.salesPrice;
//       customer.transactionCount += 1;
//       customer.totalWeight += sale.weight;
...
//       .map(customer => ({
//         ...customer,
//         averagePurchaseValue: customer.transactionCount > 0 ? customer.totalPurchases / customer.transactionCount : 0
//       }))
...
//       daily.revenue += sale.salesPrice;
//       daily.transactions += 1;
//       daily.weight += sale.weight;
//     });
...
//       .map(day => ({
//         ...day,
//         weight: Number(day.weight.toFixed(3))
//       }));
...
//         averageTransactionValue: Number(summary.averageTransactionValue.toFixed(2)),
//         averagePurchasePerCustomer: uniqueCustomerCount > 0 
//           ? Number((summary.totalRevenue / uniqueCustomerCount).toFixed(2))
//           : 0,
//         averagePricePerItem: summary.totalItems > 0 
//           ? Number((summary.totalRevenue / summary.totalItems).toFixed(2))
//           : 0
//       },
//       byMetal: {
//         gold: {
...
//           averagePurchaseValue: Number(customer.averagePurchaseValue.toFixed(2)),
...
//     // Decrypt all sales data
//     const allSales = [];
//     for (const sale of allSalesFromDB) {
//       try {
//         const decryptedData = decryptData(sale.data);
//         const decryptedSale = {
//           ...sale,
//           // Add decrypted fields to top level
//           metalType: decryptedData.metalType,
//           category: decryptedData.category,
//           purity: decryptedData.purity,
//           weight: decryptedData.weight,
...
//           customerMobile: decryptedData.customerMobile || ''
//         };
//         allSales.push(decryptedSale);
//       } catch (error) {
//         console.error("Error decrypting sale for customer report:", error);
//         // Skip corrupted sales
//       }
//     }

//     console.log("Successfully decrypted sales:", allSales.length);

//     // Group customers manually for better control
//     const customerMap = new Map();
...
//       if (sale.customerMobile && sale.customerMobile.trim() !== '') {
//         customerKey = `mobile_${sale.customerMobile.trim()}`;
//       } 
//       // Priority 2: Use name + address combination if no mobile
//       else if (sale.customerName && sale.customerName.trim() !== '') {
//         const name = sale.customerName.trim();
//         const address = sale.customerAddress ? sale.customerAddress.trim() : '';
//         customerKey = `name_${name}_${address}`;
//       }
//       // Priority 3: Use address only if no name or mobile
//       else if (sale.customerAddress && sale.customerAddress.trim() !== '') {
//         customerKey = `address_${sale.customerAddress.trim()}`;
//       }
...
//         existing.totalItems += sale.isBulk ? (sale.itemCount || 1) : 1;
//         existing.purchases.push(sale);
        
//         // Update dates
//         if (sale.soldAt < existing.firstPurchase) {
//           existing.firstPurchase = sale.soldAt;
//         }
//         if (sale.soldAt > existing.lastPurchase) {
...
//           totalItems: sale.isBulk ? (sale.itemCount || 1) : 1,
...
//     // Convert map to array and filter based on required fields
//     let customers = Array.from(customerMap.values());

//     console.log("Total unique customers before filtering:", customers.length);

//     // **FILTERING LOGIC** - Only show customers who have the selected fields
//     customers = customers.filter(customer => {
//       // Check if customer has all the required fields that are selected
//       if (includeFields.name && (!customer.customerName || customer.customerName.trim() === '')) {
//         return false;
//       }
//       if (includeFields.address && (!customer.customerAddress || customer.customerAddress.trim() === '')) {
//         return false;
//       }
//       if (includeFields.mobile && (!customer.customerMobile || customer.customerMobile.trim() === '')) {
//         return false;
//       }
...
//     // Now map the filtered customers to include only selected fields
...
//         result.averagePurchaseValue = Math.round((customer.totalPurchaseAmount / customer.transactionCount) * 100) / 100;
//         result.firstPurchase = customer.firstPurchase;
//         result.lastPurchase = customer.lastPurchase;
//       }

//       if (includeFields.purchaseItems) {
//         // Create detailed purchase items list
//         result.purchaseItems = customer.purchases.map(purchase => {
//           const metalType = purchase.metalType.charAt(0).toUpperCase() + purchase.metalType.slice(1); // Capitalize first letter
//           const category = purchase.category;
//           const weight = purchase.weight;
//           const purity = purchase.purity;
//           const itemCount = purchase.isBulk ? ` (${purchase.itemCount} items)` : '';
...
//     const totalRevenue = customers.reduce((sum, c) => sum + (c.totalPurchaseAmount || 0), 0);
...
//   try {
//     const { reportType, format, includeEntries = false } = req.body;
...
//         const decryptedMetadata = decryptData(metadata.data);
...
//         // Build categories with original keys
//         const categories = {};
...
//           reportData = {
//             totalGross: decryptedMetadata.totalGoldGross || 0,
//             totalPure: decryptedMetadata.totalGoldPure || 0,
//             averagePurity: decryptedMetadata.totalGoldGross > 0 
//               ? Math.floor((decryptedMetadata.totalGoldPure / decryptedMetadata.totalGoldGross) * 10000) / 100
//               : 0,
//             categories: goldCategories,
//             entries: goldEntries
//           };
...
//           reportData = {
//             totalGross: decryptedMetadata.totalSilverGross || 0,
//             totalPure: decryptedMetadata.totalSilverPure || 0,
//             averagePurity: decryptedMetadata.totalSilverGross > 0 
//               ? Math.floor((decryptedMetadata.totalSilverPure / decryptedMetadata.totalSilverGross) * 10000) / 100
//               : 0,
//             categories: silverCategories,
...
//           reportData = {
//             totalGoldGross: decryptedMetadata.totalGoldGross || 0,
//             totalGoldPure: decryptedMetadata.totalGoldPure || 0,
//             totalSilverGross: decryptedMetadata.totalSilverGross || 0,
//             totalSilverPure: decryptedMetadata.totalSilverPure || 0,
//             goldAveragePurity: decryptedMetadata.totalGoldGross > 0 
//               ? Math.floor((decryptedMetadata.totalGoldPure / decryptedMetadata.totalGoldGross) * 10000) / 100
//               : 0,
//             silverAveragePurity: decryptedMetadata.totalSilverGross > 0 
//               ? Math.floor((decryptedMetadata.totalSilverPure / decryptedMetadata.totalSilverGross) * 10000) / 100
//               : 0,
...
//         // Build report data structure that matches your existing format
//         reportData = {
//           dateRange: {
//             startDate: startDate || null,
//             endDate: endDate || null
...
//       case 'customers':
//         const { fields, fieldSelection } = req.body;
...
//                 weight: sale.weight,
//                 purity: sale.purity,
//                 itemCount: sale.isBulk ? (sale.itemCount || 1) : 1,
//                 isBulk: sale.isBulk,
...
//             // Always include for calculations
...
//             averageCustomerValue: customers.length > 0 
//               ? Math.round(customers.reduce((sum, customer) => sum + (customer.totalPurchaseAmount || 0), 0) / customers.length)
//               : 0,
//             topSpendingCustomer: customers.length > 0 ? customers[0] : null,
//             averageTransactionsPerCustomer: customers.length > 0
//               ? Math.round(customers.reduce((sum, customer) => sum + (customer.transactionCount || 0), 0) / customers.length)
//               : 0,
...
// //     const allSales = await Sales.find({ userId: req.userId }).lean();
...
// //     // **NEW FILTERING LOGIC** - Only show customers who have the selected fields
// //     customers = customers.filter(customer => {
// //       // Check if customer has all the required fields that are selected
// //       if (includeFields.name && (!customer.customerName || customer.customerName.trim() === '')) {
// //         return false;
...
// //         if (metadata.categoryTotals) {
// //           Object.entries(metadata.categoryTotals).forEach(([categoryKey, categoryData]) => {
// //             categories[categoryKey] = {
// //               grossWeight: categoryData.grossWeight || 0,
// //               pureWeight: categoryData.pureWeight || 0,
// //               totalItems: categoryData.totalItems || 0,
...
// //             totalGross: metadata.totalGoldGross || 0,
// //             totalPure: metadata.totalGoldPure || 0,
// //             averagePurity: metadata.totalGoldGross > 0 
// //               ? Math.floor((metadata.totalGoldPure / metadata.totalGoldGross) * 10000) / 100
// //               : 0,
// //             categories: goldCategories,
// //             entries: goldEntries
...
// //             totalGross: metadata.totalSilverGross || 0,
// //             totalPure: metadata.totalSilverPure || 0,
// //             averagePurity: metadata.totalSilverGross > 0 
// //               ? Math.floor((metadata.totalSilverPure / metadata.totalSilverGross) * 10000) / 100
// //               : 0,
// //             categories: silverCategories,
// //             entries: silverEntries
...
// //           // Full report - all categories
// //           reportData = {
// //             totalGoldGross: metadata.totalGoldGross || 0,
// //             totalGoldPure: metadata.totalGoldPure || 0,
// //             totalSilverGross: metadata.totalSilverGross || 0,
// //             totalSilverPure: metadata.totalSilverPure || 0,
// //             goldAveragePurity: metadata.totalGoldGross > 0 
// //               ? Math.floor((metadata.totalGoldPure / metadata.totalGoldGross) * 10000) / 100
// //               : 0,
// //             silverAveragePurity: metadata.totalSilverGross > 0 
// //               ? Math.floor((metadata.totalSilverPure / metadata.totalSilverGross) * 10000) / 100
// //               : 0,
// //             categories: categories,
// //             entries: [...goldEntries, ...silverEntries]
...
// //       // Updated case 'sales' in the download endpoint - cleaned and optimized
// // case 'sales':
// //   const { startDate, endDate } = req.body;
...
// //   // Get individual sales entries if includeEntries is true
// //   let salesEntries = [];
// //   if (includeEntries) {
// //     salesEntries = await Sales.find(dateFilter)
// //       .sort({ soldAt: -1 })
// //       .lean();
// //   }
...
// //           purity: "$purity"
// //         },
// //         totalSalesAmount: { $sum: "$salesPrice" },
...
// //   const summary = totalSales[0] || {
// //     totalRevenue: 0,
// //     totalItems: 0,
// //     totalWeight: 0
// //   };
  
// //   // Build report data structure that matches PDF exactly
// //   reportData = {
// //     dateRange: {
// //       startDate: startDate || null,
// //       endDate: endDate || null
...
// //     })),
// //     // Only include entries if requested
// //     entries: includeEntries ? salesEntries : []
// //   };
  
// //   reportTitle = 'Sales Report';
// //   break;
        
// //       // Updated customer case for the backend download endpoint
// //       // Updated customer case for the backend download endpoint
// // case 'customers':
// //   const { fields, fieldSelection } = req.body;
  
// //   try {
// //     // Get all sales data to build customer information with detailed transaction data
...
// //           },
// //           totalPurchaseAmount: { $sum: "$salesPrice" },
// //           transactionCount: { $sum: 1 },
// //           lastPurchaseDate: { $max: "$soldAt" },
// //           transactions: {
// //             $push: {
// //               date: "$soldAt",
// //               amount: "$salesPrice",
...
// //             $round: [
// //               { $divide: ["$totalPurchaseAmount", "$transactionCount"] },
// //               2
// //             ]
// //           }
// //         }
...
// //     // Transform and filter data based on selected fields
// //     const customers = customerSales.map(customer => {
// //       const customerData = {};
      
// //       // Always include basic data but filter based on fieldSelection
// //       if (!fieldSelection || fieldSelection.includeName) {
// //         customerData.customerName = customer._id.name || 'N/A';
// //       }
// //       if (!fieldSelection || fieldSelection.includeMobile) {
// //         customerData.customerMobile = customer._id.mobile || 'N/A';
// //       }
// //       if (!fieldSelection || fieldSelection.includeAddress) {
// //         customerData.customerAddress = customer._id.address || 'N/A';
// //       }
// //       if (!fieldSelection || fieldSelection.includePurchaseAmount) {
// //         customerData.totalPurchaseAmount = customer.totalPurchaseAmount || 0;
// //       }
// //       if (!fieldSelection || fieldSelection.includePurchaseItems) {
// //         // Include detailed transaction information instead of just count
// //         customerData.transactions = customer.transactions || [];
// //         customerData.transactionCount = customer.transactionCount || 0;
// //       }
      
// //       // Always include for calculations
// //       customerData.averagePurchaseValue = customer.averagePurchaseValue || 0;
// //       customerData.lastPurchaseDate = customer.lastPurchaseDate || null;
...
//     // Calculate averages for each metal with proper rounding and null safety
//     // const goldAvgPurity = totalGoldGross > 0 
//     //   ? Math.floor((totalGoldPure / totalGoldGross) * 10000) / 100
//     //   : 0;
    
//     // const silverAvgPurity = totalSilverGross > 0 
//     //   ? Math.floor((totalSilverPure / totalSilverGross) * 10000) / 100
//     //   : 0;
// // Replace this section in the stock report
// const goldAvgPurity = totalGoldGross > 0 
//   ? safeNumber((totalGoldPure / totalGoldGross) * 100, 0, 2)
//   : safeNumber(0, 0, 2);

// const silverAvgPurity = totalSilverGross > 0 
//   ? safeNumber((totalSilverPure / totalSilverGross) * 100, 0, 2)
//   : safeNumber(0, 0, 2);
...
//           // Replace this line in the category processing forEach loop
// const averagePurity = grossWeight > 0 
// ? safeNumber((pureWeight / grossWeight) * 100, 0, 2)
// : safeNumber(0, 0, 2);
...
//             averagePurity: Number(averagePurity.toFixed(2))
//           };
...
router.get("/sales", async (req, res) => {
  try {
    ...
    // Convert map to array and calculate additional fields with safe math
    // const salesAggregation = Array.from(salesAggregationMap.values()).map(item => ({
    //   ...item,
    //   averagePrice: item.salesCount > 0 ? safeNumber(item.totalSalesAmount / item.salesCount) : 0,
    //   totalPureWeight: safeNumber(item.totalGrossWeight * (item._id.purity / 100)),
    //   uniqueCustomers: item.customers.size,
    //   customers: Array.from(item.customers)
    // }));
    const salesAggregation = Array.from(salesAggregationMap.values()).map(item => ({
    ...
    // const summary = {
    //   totalRevenue: decryptedSales.reduce((sum, sale) => sum + safeNumber(sale.salesPrice), 0),
    //   totalTransactions: decryptedSales.length,
    //   totalWeight: decryptedSales.reduce((sum, sale) => sum + safeNumber(sale.weight), 0),
    ...
    //       safeNumber(customer.totalPurchases / customer.transactionCount, 0, 2) : 0
    //   }))
    //   .sort((a, b) => b.totalPurchases - a.totalPurchases);
    ...
    const salesReport = {
      ...
      //   uniqueCustomers: uniqueCustomerCount,
      //   averageTransactionValue: safeNumber(summary.averageTransactionValue, 0, 2),
      //   averagePurchasePerCustomer: uniqueCustomerCount > 0 
      //     ? safeNumber(summary.totalRevenue / uniqueCustomerCount, 0, 2)
      //     : 0,
      //   averagePricePerItem: summary.totalItems > 0 
      //     ? safeNumber(summary.totalRevenue / summary.totalItems, 0, 2)
      //     : 0
      // },
      summary: {
        totalRevenue: safeNumber(summary.totalRevenue),
