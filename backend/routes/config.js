...
//     if (!metadata || !metadata.categoryTotals) {
...
//     if (!metadata || !metadata.categoryTotals || !metadata.categoryTotals[categoryKey]) {
//       return res.json({ 
//         metalType: metal.toLowerCase(), 
...
//     // CRITICAL FIX: Add markModified for nested objects
//     metadata.markModified('categoryTotals');
//     await metadata.save();
...
//     const metadata = await Metadata.findOne({ userId });
//     if (!metadata || !metadata.categoryTotals || !metadata.categoryTotals[oldCategoryKey]) {
//       console.log("❌ Old category not found");
//       return res.status(404).json({ error: "Category not found" });
//     }

//     // Check if new category name already exists (unless it's the same name)
//     if (oldCategoryKey !== newCategoryKey && metadata.categoryTotals[newCategoryKey]) {
...
//     // Update metadata - rename the key if needed
//     if (oldCategoryKey !== newCategoryKey) {
//       console.log("🔄 Moving category data to new key");
//       metadata.categoryTotals[newCategoryKey] = metadata.categoryTotals[oldCategoryKey];
//       delete metadata.categoryTotals[oldCategoryKey];
//     }
    
//     metadata.categoryTotals[newCategoryKey].categoryName = trimmedNewName;
...
//     // Only rebuild metadata if there were actual entries updated
//     if (updatedEntries > 0) {
//       await updateMetadata(userId);
//     }
...
//     if (!metadata.categoryTotals[categoryKey].purities) {
//       metadata.categoryTotals[categoryKey].purities = {};
//     }

//     const oldPurityKey = oldPurity.toString();
//     const newPurityKey = newPurity.toString();
    
//     if (!metadata.categoryTotals[categoryKey].purities[oldPurityKey]) {
//       return res.status(404).json({ error: "Old purity level not found" });
//     }

//     // Check if new purity already exists (unless it's the same)
//     if (oldPurityKey !== newPurityKey && metadata.categoryTotals[categoryKey].purities[newPurityKey]) {
//       return res.status(400).json({ error: "New purity level already exists" });
//     }
...
//     // Update metadata - rename the purity key if needed
//     if (oldPurityKey !== newPurityKey) {
//       metadata.categoryTotals[categoryKey].purities[newPurityKey] = 
//         metadata.categoryTotals[categoryKey].purities[oldPurityKey];
//       delete metadata.categoryTotals[categoryKey].purities[oldPurityKey];
//     }
...
//     console.log("✅ Purity updated successfully");
...
//     if (!metadata.categoryTotals[categoryKey].purities[purityKey]) {
...
//     delete metadata.categoryTotals[categoryKey].purities[purityKey];
...
//     // Check if user has metadata document
...
//     // Force mark as modified and save
//     metadata.markModified('categoryTotals');
//     await metadata.save();
//     console.log("✅ Metadata saved successfully");

//     // Verify the save by fetching it back from database
//     const verifyMetadata = await Metadata.findOne({ userId });
...
// CHAIN POSITION: 5.1/11 (HOTFIX)

// DEPENDENCIES: utils/encrypt.js, models/Metadata.js, routes/metadata.js
// EXPORTS: All config routes updated to work with encrypted metadata
...
// TESTING: 
// 1. Test GET /api/config/categories/gold - should return decrypted categories
// 2. Test POST /api/config/categories - should create new category with encryption
// 3. Test category deletion and renaming with encrypted metadata
// 4. Test purity operations with encrypted metadata
// 5. Verify all operations work seamlessly with encrypted data